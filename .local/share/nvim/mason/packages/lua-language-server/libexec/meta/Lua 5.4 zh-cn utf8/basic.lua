...
---| "step"         # 执行一次垃圾收集的“步骤”。此选项后可以带一个额外参数 `size`（整数）。...如果 `size` 为正的 n，收集器的行为就像新分配了 n 个字节；如果为 0，则执行一个“基本步骤”。...在增量模式下，基本步骤对应当前的步进大小；在分代模式下，基本步骤会执行一次完整的次要收集，...或在收集器已安排增量步骤时执行该增量步骤。...在增量模式下，如果该步骤完成了一个收集周期，函数返回 `true`；在分代模式下，如果该步骤完成了一个主要收集，返回 `true`。...
---| "isrunning"    # 返回表示收集器是否在工作的布尔值。
---| "incremental"  # 将收集器模式改为“增量模式”，并返回之前的模式（可能是 `"generational"` 或 `"incremental"`）。
---| "generational" # 将收集器模式改为“分代模式”，并返回之前的模式（可能是 `"generational"` 或 `"incremental"`）。
